...
function MiniDiaryDB() {
    ...
    async function initializeCounter() {
        ...
        const existing = await db.collection("counters").findOne({ _id: "diaryEntryId" });
        if (!existing) {
            await db.collection("counters").insertOne({ _id: "diaryEntryId", sequence_value: 0 });
        }
    ...
    async function getNextSequenceValue() {
        ...
        client.close();
        if (!sequenceDocument || (sequenceDocument && !("sequence_value" in sequenceDocument))) {
    ...
    miniDiaryDB.getDiaryEntryById = async (id) => {
        const { client, db } = await connect();
        try {
            return await db.collection(DIARIES_COLLECTION).findOne({ primeId: parseInt(id, 10) });
        } finally {
            client.close();
        }
    };
    ...
    miniDiaryDB.updateDiaryEntryById = async (id, update) => {
        const { client, db } = await connect();
        try {
            await db.collection(DIARIES_COLLECTION).updateOne(
    ...
    miniDiaryDB.deleteDiaryEntryById = async (id) => {
        const { client, db } = await connect();
        try {
            return await db.collection(DIARIES_COLLECTION).deleteOne({ primeId: parseInt(id, 10) });
        } finally {
            client.close();
        }
    };
    ...
    miniDiaryDB.getDiaryEntriesByUserId = async (userid) => {
        const { client, db } = await connect();
        try {
            return await db.collection(DIARIES_COLLECTION).find({ userid: userid }).toArray();
        } finally {
            client.close();
        }
    };
